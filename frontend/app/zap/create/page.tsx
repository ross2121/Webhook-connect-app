...
function useAvailableActionsAndTriggers() {
    ...
    useEffect(() => {
        axios.get(`${BACKEND_URL}/v1/trigger/triggers`,{
            headers: {
                Authorization: localStorage.getItem("token")
            }
        })
            .then(x => setAvailableTriggers(x.data.triggers))
        axios.get(`${BACKEND_URL}/v1/action/action`,{
            headers: {
                Authorization: localStorage.getItem("token")
            }
        }
        )
            .then(x => setAvailableActions(x.data.triggers))
    }, [])
    ...
    return <div>
        ...
        <div className="flex justify-end bg-slate-200 p-4">
            <PrimaryButton onClick={async () => {
             ...
             await axios.post(`${BACKEND_URL}/v1/zap/zaps`, {
                    "availableTriggerid": selectedTrigger.id,
                    "metadata": {},
                    "action": selectedActions.map(a => ({
                        availabaction: a.availableActionId,
                        metadata: a.metadata
                    }))
                }, {
                    headers: {
